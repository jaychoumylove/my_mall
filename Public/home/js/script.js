(function (window, document, $) {
	...
	$(document).ready(function($) {
		...
		/* .cart-list */
		/*if($(".mini-cart-wrap .widget-shopping-cart-content .cart-list").children("li").length>3){
			
		    $(".mini-cart-wrap .widget-shopping-cart-content .cart-list li.none").css("display","block");
		}*/
		
		
		$('.check-p p').on('click', function() {
			$(this).addClass("onitem").siblings().removeClass("onitem");
		});
		
		
		$('.single-comment-form .zx_cdR01 ul li i').on('click', function() {
	...
	if ($(".client-carousel").length > 0) {
		$(".client-carousel").each(function(){
			...
			$(this).owlCarousel({
				...
				smartSpeed: 800,
				autoplayHoverPause: true,
				/*设置浏览器宽度和幻灯片可见个数，格式为[X,Y]，X 为浏览器宽度，Y 为可见个数，如[1199,4]就是如果浏览器宽度小于1199，每页显示 4 张，*/
